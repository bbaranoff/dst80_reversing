...
#define KEY40                 0xFFFFFFFFFFUL

// ---- cipher DST80 (identique a tes kernels, deja valide) ----
...
// =====================================================================
//  DERIVATION CONSTRUCTEUR  >>> LE SEUL BLOC A REMPLACER <<<
//
//  Produit (KL, KR) a partir du serial PUBLIC + des constantes SECRETES.
//  Point cle : KR n'est PAS revcomp(KL) ici. C'est tout l'interet de ce
//  kernel par rapport a reverse/fast/constructor -- on ne suppose plus la
//  diagonale revcomp, on modelise une vraie derivation serial-dependante.
//
//  Le corps ci-dessous est un PLACEHOLDER ILLUSTRATIF. Il montre la
//  signature et le flux, il NE CORRESPOND A AUCUN CONSTRUCTEUR REEL.
//  Remplace-le par la derivation reverse-engineeree (Wouters et al. 2020,
//  TCHES 2020-2) du constructeur vise. Le serial etant connu, on ne brute
//  QUE c0/c1/c2 -> 2^24 (etendre a 4 octets = 2^32 si besoin).
// =====================================================================
static inline void derive_keys(ulong serial, ulong c0, ulong c1, ulong c2,
                               ulong* kl_out, ulong* kr_out)
{
    ulong s0 = (serial >>  0) & 0xFF;
    ulong s1 = (serial >>  8) & 0xFF;
    ulong s2 = (serial >> 16) & 0xFF;
    ulong s3 = (serial >> 24) & 0xFF;
    ulong s4 = (serial >> 32) & 0xFF;

    // --- PLACEHOLDER (a remplacer par la vraie derivation) ---------------
    ulong kl = ((s4 ^ c0) << 32) | ((s3 ^ c1) << 24) | ((s2 ^ c2) << 16)
             | (s1 << 8) | s0;
    ulong kr = (c0 << 32) | (c1 << 24) | (c2 << 16)
             | ((s3 ^ s1) << 8) | (s4 ^ s0);
    // --- fin placeholder -------------------------------------------------
...
__kernel void dst80_search(
    const ulong serial,        // serial PUBLIC, fixe (lu sur le transpondeur)
    const ulong global_base,
    const ulong challenge1,
    const uint  target1,
    const ulong challenge2,
    const uint  target2,
    __global ulong* out_matches,  // stride 3 : [constantes, kl, kr]
    __global uint*  out_count
){
    size_t gid = get_global_id(0);
    ulong idx = global_base + gid;

    // 3 octets de constantes constructeur inconnues -> espace 2^24
    ulong c0 = (idx >>  0) & 0xFFUL;
    ulong c1 = (idx >>  8) & 0xFFUL;
    ulong c2 = (idx >> 16) & 0xFFUL;

    ulong kl, kr;
    derive_keys(serial, c0, c1, c2, &kl, &kr);

    ulong res1 = simulate_dst80(kl, kr, challenge1);
    if ((uint)(res1 & 0xFFFFFFUL) == target1) {
        ulong res2 = simulate_dst80(kl, kr, challenge2);
        if ((uint)(res2 & 0xFFFFFFUL) == target2) {
            uint pos = atomic_inc(out_count);
            if (pos < 100) {
                out_matches[pos * 3 + 0] = (c2 << 16) | (c1 << 8) | c0; // constantes
                out_matches[pos * 3 + 1] = kl;
                out_matches[pos * 3 + 2] = kr;
            }
        }
    }
}
