...
# 🛠 DST80 Signature Generator Tutorial

This script calculates the **24-bit response** (signature) for a given DST80 challenge. It automatically handles the internal symmetry of the cipher by deriving **Key Right (KR)** from your provided **Key Left (KL)**.

### 1. How the Key is Constructed

The script assumes the common automotive implementation where the 80-bit key is mirrored:

* **KeyL (40 bits):** Provided by you (or uses default).
* **KeyR (40 bits):** Automatically generated by inverting each byte of KeyL ().
...
python generate.py --kl 0xdeadbeef00 --challenge 0xcafebabe01
   DST80 Generation Results    
 Key Left (KL)    0xdeadbeef00 
 Key Right (KR)   0xff10415221 
...
The script will print a clean summary in your terminal:

* **KeyL**: The source 40-bit key.
